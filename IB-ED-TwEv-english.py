# TweetEval Classification with IndoBERT-lite-base-p1 (Token already saved) with Emoji Description in English
...
## Load Tokenized Data
train_df = pd.read_csv("dataset-english/TwEv/TTVED/with_emoji_train.csv")
val_df = pd.read_csv("dataset-english/TwEv/TTVED/with_emoji_val.csv")
test_df = pd.read_csv("dataset-english/TwEv/TTVED/with_emoji_test.csv")
...
model = TFAutoModelForSequenceClassification.from_pretrained(
    "indobenchmark/indobert-lite-base-p1", num_labels=1
)
...
for batch_size, lr, epochs in itertools.product(batch_sizes, learning_rates, epoch_list):
    ...
    temp_model = TFAutoModelForSequenceClassification.from_pretrained(
        "indobenchmark/indobert-lite-base-p1", num_labels=1
    )
...
loss_df.to_csv("model/TwEv_indobert_loss_history.csv", index=False)
...
## Save Trained Model
model.save_pretrained("model/TwEv_indobert_with_emoji")
